...
값을 못 찾는 경우의 $T_u$는 간단히 구할 수 있습니다.
이 때의 `get()` 연산은 해시 값을 계산해 슬롯에 접근하고, 체인의 모든 노드마다 키를 확인합니다.
그런데 체인의 길이는 기댓값이 $\alpha$ 이므로, 노드의 키를 확인하는 시간 $c$에 대해 $T_u = c_k + c \alpha$, 즉 $\Theta(1 + \alpha)$ 또는 $\Theta(n)$을 얻습니다.
...
그 개수를 $X$라고 합시다.
그러면 값을 찾은 경우의 시간 복잡도 $T_s$는, 반대의 경우의 $T_u$처럼, $T_s = c_k + c X$가 됩니다.
...
해시 값도 서로 독립이므로, $L_i S_i$의 기댓값 $C_i$는 다음과 같습니다.

$$
\begin{align*}
  C_i &= E [ L_{i} S_i ] \\
      &= \sum_{k=0}^m \sum_{j=i+1}^n E [ I_{ik} I_{jk} S_i ] \\
...
이를 모든 값의 순번에 대해 더하면, 구하고자 하는 $X$를 얻습니다.

$$
\begin{align*}
  X = \sum_{i=1}^n C_i &= \frac{1}{mn} \left( n^2 - \frac{n(n+1)}{2} \right) \\
                   &= \frac{\alpha}{2} - \frac{1}{2m}
\end{align*}
$$

따라서 시간 복잡도 $T_s$는 $c_k + c\alpha/2 - c/2m$이고, $\Theta(1 + \alpha)$ 또는 $\Theta(n)$을 얻습니다.
...
`get()`의 소요 시간은 배열의 크기에 상관 없이 일정 수준 이하의 소요 시간을 보입니다.
$n$ 개의 값을 채우는 `set()`의 경우, $\Theta(1)$의 시간이 걸리는 연산을 $n$ 번 수행하므로 $\Theta(n)$의 시간이 걸려야 하고, 실제로도 이를 따르는 것으로 보입니다.
