...
이런 형태는 삼단논법<L>syllogism</L>이라는 이름을 갖게 되었습니다.
...
하나의 대수 구조<L>algebraic structure</L>로 정리했다는 편이 더 정확할 것입니다.
...
여기서는 참과 거짓 외에 모름<L>unknown</L> 이라는 값도 더한 쓰리 밸류 로직<L>three-valued logic</L>을 이용합니다.
아니면 퍼지 로직<L>fuzzy logic</L>처럼 $0$과 $1$ 사이의 무한히 많은 숫자를 쓰는 경우도 있는데요.
신경망<L>nueral networks</L>을 이용한 인공지능에서 이를 사용하기도 합니다.
이렇게 다른 종류의 논리 또한 컴퓨터 과학에서 적용되고 있습니다.
...
이는 1차 논리학<L>first-order logic</L>이라고 불립니다.
수학에서 명제를 표현할 때 흔히 사용되고, $\forall$과 $\exists$와 같은 기호가 그런 수량자를 나타냅니다.
...
(자세한 내용에 흥미가 있다면 컴퓨터 구조를 다루는 자료에서 확인할 수 있습니다.)
