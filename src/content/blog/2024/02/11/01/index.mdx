...
이를 위한 자료 구조를 인바이런먼트<AltLang>environment</AltLang>라고 부를 것입니다.
...
이는 변수 섀도잉<AltLang>variable shadowing</AltLang>이라고 불립니다.

<FigureV2 src={fig4} alt="variable shadowing">
...
이런 함수를 커리된 함수<AltLang>curried function</AltLang>라고 부릅니다.
...
그래서 그 함수가 일종의 프라이빗<AltLang>private</AltLang> 변수를 가졌다고 볼 수 있는데요.
이런 함수를 클로저라고 부릅니다.
여기서 클로저가 변수 `a`를 캡처<AltLang>capture</AltLang>했다고도 표현합니다.
