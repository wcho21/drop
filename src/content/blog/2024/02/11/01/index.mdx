...
<PostLink href="/2024/02/10/01">이전 글</PostLink>에서 간단한 프로그래밍 언어를 위한 파서<L>parser</L>를 만들었습니다.
파서는 소스 코드를 문법적으로 해석한 결과로 추상 문법 트리<L>abstract syntax tree</L>를 만드는데요.
이 글에서는 트리를 받아 값으로 평가<L>evaluate</L>하는 이밸류에이터<L>evaluator</L>를 만들 것입니다.
...
이를 통해 함수의 커링<L>currying</L>을 지원할 수 있게 됩니다.
...
여기서 스트링 표현을 위한 메소드 `__repr__()`는 나중에 `print()`로 확인할 용도로 만든 것입니다.
앞으로의 경우도 그렇습니다.
...
여기서 만들 인터프리터는 `a = 1`과 같은 대입 연산을 평가하면, 변수에 값을 대입하고 평가 값은 그 대입한 값입니다.
...
이런 확장은 인바이런먼트의 단방향 연결 리스트<L>singly linked list</L>를 만들 것입니다.
...
그런데 여기서 변수 `a`는 함수 외부에서 접근할 방법이 없게 됩니다.
그래서 그 함수가 일종의 프라이빗<L>private</L> 변수를 가졌다고 볼 수 있는데요.
이런 함수를 클로저라고 부릅니다.
여기서 클로저가 변수 `a`를 캡처<L>capture</L>했다고도 표현합니다.
