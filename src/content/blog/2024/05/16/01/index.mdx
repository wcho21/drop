...
한편, `Comparator` 인터페이스의 메소드가 읽기 불편한 감이 있어서 비교하는 부분을 유틸 메소드로 분리했습니다.
...
이를 통해, 카드를 찾는 횟수가 많아서 카드 개수를 넘는다면, 소트를 해놓는 것이 유리하다는 결론이 나옵니다.
곧 알아볼 소팅 알고리즘은 최악의 경우 $\Theta(f(n)) = \Theta(n^2)$으로, 소트해놓는 것이 $\Theta(f(n) + m\lg n) = \Theta(n^2) < \Theta(mn)$으로서 더 나은 시간 복잡도를 가집니다.
...
측정 결과를 통해, 셸 소트는 최선의 경우에서 인서션 소트보다 불리하지만, 최악의 경우는 유리함을 확인할 수 있습니다.
