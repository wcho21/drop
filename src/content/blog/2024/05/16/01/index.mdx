...
그 방법인 바이너리 서치<L>binary search</L>라는 알고리즘은 데이터가 순서대로 있다는 사실을 서치에 활용합니다.
이것이 알아볼 첫 번째 알고리즘입니다.

반면, 데이터가 소트되어 있는지 모른다면, 매번 시퀀셜 서치를 이용해야할 것입니다.
...
이것이 알아볼 두 번째 알고리즘입니다.
이 알고리즘이 어떤 경우에 느린지 분석해보고, 이를 바탕으로 셸 소트<L>Shell sort</L> 알고리즘으로 개선해보겠습니다.
...
이렇게 하면, 구간이 비었다는 사실은 두 끝이 같다는 것, 즉 $\textit{begin} = \textit{end}\,$로 표현할 수 있습니다.
반대로, 구간이 비어있지 않다는 것은 $\textit{begin} < \textit{end}\,$와 같게 됩니다.
...
- 서치 대상 $\textit{target}\,$이 배열 $\textit{arr}\,$에 있다면, 그 인덱스 $i$는 $[\textit{begin}, \textit{end})$ 안에 있다.
...
여기에 변수 초기화와 리턴문을 더 실행하는 전체 코드 또한 $\Theta(\lg n)$을 가집니다.
...
```java
public class Sorter {
  public static <T> T[] sortArray(T[] arr, int begin, int end,
      SortStrategy<T> strat, Comparator<T> comp) {
...
```java
  public static <T extends Comparable<? super T>> T[] sortArray(T[] arr,
      SortStrategy<T> strat) {
...
<Alg>
  ...
  <ABlock>
    ...
    <AWhile>$gap < \lfloor(\textit{end}-\textit{begin})/3\rfloor$ 인 </AWhile>
    <ABlock>
    ...
    <ABlock>
      <p><AC>// 간격 $\textit{gap}\,$으로 서브 배열을 인서션 소트</AC></p>

      <AForeach>$\textit{begin}+\textit{gap}\,$부터 $\textit{end}\,$전까지인 $i$</AForeach>
      <ABlock>
        $j \leftarrow i-\textit{gap}$ <AC>// 서브 배열 상 바로 왼쪽 데이터 인덱스</AC>

        <p><AC>// 오른쪽($j+gap$)이 바로 왼쪽($j$)과 자리가 안맞는 동안</AC></p>

        <AWhile>$j \geq \textit{begin}$ 이고 $arr$[$j$] $> arr$[$j+\textit{gap}$] 인 </AWhile>
        <ABlock>
...
이렇게 구현한 셸 소트는, 다음 테스트 코드처럼 스트레터지를 넘겨줌으로써 사용할 수 있습니다.
