...
한편, 데이터 구조는 관심있는 문제를 빠르게 해결하기 위해 데이터를 정리하는 방법입니다.
...
다시 말해, 랜덤 엑세스를 희생해 크기의 제약에서 벗어난 것입니다.
...
그러면 연결 리스트가 효율적으로 해낼 수 있는, 즉 $\Theta(1)$이 걸리는 작업을 ADT로 정리해봅시다.
그리고 사용자가 데이터에 집중할 수 있도록 노드의 존재를 감춥시다.
...
그러나 (바람직하지는 않지만) 배열을 엄청나게 크게 만들었다고 하고, 그래서 크기가 문제되지 않는다면, 배열로도 스택을 구현할 수 있습니다.
...
C나 C++ 같은 프로그래밍 언어에서는 주소 값을 직접적으로 다루는 기능을 포인터<L>pointer</L>라는 데이터 타입으로 지원합니다.
이와 달리 이런 기능이 없는 언어에서도, 내부적으로는 주소 값을 다룰 수밖에 없게 됩니다.

## 다이나믹 배열

크기의 제약이 없다면 배열로도 스택을 구현할 수 있는데요.
...
이렇게 늘리는 비율을 그로스 팩터<L>growth factor</L>라고 하고, 여기선 $2$가 됩니다.
...
이는 비교적 시간이 걸리는 일입니다.
만약, 그로스 팩터가 $4$라면, 늘리는 일은 덜 일어나겠지만, 그 직후에 실제로 사용하는 메모리는 25%에 불과하므로 낭비로 볼 수 있습니다.
...
시간 복잡도를 구할 때는 배열 할당이 $\Theta(1)$이라고 가정했지만, 배열의 크기가 크면 실제로는 그렇지 않기 때문입니다.
