...
그런데 컴퓨터가 이런 재귀 알고리즘은 어떻게 수행할 수 있을까요?
실제로는 이를 위해 스택<L>stack</L>이라고 하는 데이터 구조<L>data structure</L>를 암묵적으로 사용합니다.
...
컴퓨터는 재귀를 수행하기 위해 이런 스택을 사용합니다.
왜냐면 베이스 케이스를 수행하고 나서 리턴 값을 계속 직전의 재귀 케이스로 전달해야 하기 때문입니다.
따라서 매번 직전의 재귀로 돌아갈 필요가 있기 때문에 스택이 필요하게 됩니다.
...
한편, 재귀의 수행을 함수의 호출로 볼 수 있으므로, 이때 쓰이는 스택을 특히 콜 스택<L>call stack</L>이라고도 부릅니다.
따라서 어느 재귀 알고리즘이든, 재귀 없이 반복문만 사용하는 반복 알고리즘<L>iterative algorithm</L>으로 바꿀 수 있습니다.
이 콜 스택을 알고리즘에서 직접 따라하면 되기 때문입니다.
달리 말하면, 우리가 재귀 알고리즘을 편하게 사용할 수 있는 이유는 컴퓨터가 대신 콜 스택을 관리해주기 때문입니다.

콜 스택을 직접 따라하는 일은 복잡하지만, 예시로 든 피보나치 수 알고리즘의 경우 그럴 필요는 없습니다.
...
<Alg>
  ...
  <ABlock>
    ...
    <AK>다음을</AK> 스택이 비어있지 않은 <AK>동안 반복</AK>
    <ABlock>
...
이때 처음에 $n=3$이 주어집니다.
이는 그대로 스택에 들어갑니다.
반복문에서 $m$으로 다시 꺼낸 이 숫자는 $m \geq 3$을 만족하므로, 스택에는 $1$과 $2$가 차례로 들어갑니다.
이 두 숫자는 스택에서 나올 때마다 반복문에서 $\textit{fib}$ 변수를 하나씩 증가시킵니다.
따라서 결과적으로 $\textit{fib}=2$를 리턴하게 되고, 실제로 세 번째 피보나치 수 $f_3$을 얻게 됩니다.
...
자바에는 오토박싱<L>autoboxing</L>이라는 기능으로 이 두 타입 간의 변환을 도와주지만, `int` 타입을 `Integer` 타입으로 바꾸는 것은 원시 타입을 그대로 쓰는 것보다 성능 상 불리한 점이 있습니다.
...
<Figure src={fig7} alt="elapsed time for prepend and remove">
  <FigureCaption slot="caption">그림 7. 각 시나리오에 따른 소요 시간. 직선은 선형 회귀선.</FigureCaption>
</Figure>
...
<Figure src={fig12} alt="elapsed time for append and remove">
  <FigureCaption slot="caption">그림 12. 각 시나리오에 따른 소요 시간. 직선은 선형 회귀선.</FigureCaption>
</Figure>

여기서도 노드의 개수 $n$과 $n$번 연산에 걸리는 소요 시간 사이에 선형적인 관계를 확인할 수 있습니다.
따라서 시간 복잡도 분석이 유효함을 알 수 있습니다.
