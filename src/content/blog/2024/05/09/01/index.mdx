...
이 스택을 알고리즘에서 직접 만들면 되기 때문입니다.
달리 말하면, 우리가 재귀 알고리즘을 편하게 사용할 수 있는 이유는 컴퓨터가 대신 이런 스택을 관리해주기 때문입니다.

컴퓨터가 관리하는 스택을 직접 따라하는 일은 복잡하지만, 예시로 든 피보나치 수 알고리즘의 경우 그럴 필요는 없습니다.
...
여기서는 [JUnit 5 프레임워크][junit5]를 사용합니다.
