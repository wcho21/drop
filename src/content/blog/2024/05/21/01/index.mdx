...
import fig12 from "@texfigs/2024/05/21/01/fig12.svg";
import fig13 from "@texfigs/2024/05/21/01/fig13.svg";
...
무게를 모르는 추의 순서를, 몇 번의 비교 만에 알아낼 수 있을까요?

양쪽에 하나씩 추를 달 수 있는 저울이 있고, 추를 무게 순으로 정리하고 싶다고 해봅시다.
그렇다면 저울을 얼마나 적게 쓸 수 있는지가 중요한 문제가 됩니다.
왜냐면 비교를 통해 데이터를 순서대로 소트<L>sort</L>하는 비교 기반 소팅 알고리즘<L>comparison-based sorting algorithm</L>이 얼마나 빠를 수 있는지와 관련이 있기 때문입니다.

예를 들어 추가 세 개일 때를 봅시다.

<Figure src={fig1} alt="scale with three weights">
  <FigureCaption slot="caption">그림 1. 저울을 이용해 정하는 추의 무게 순서.</FigureCaption>
</Figure>

생각해보면 저울을 최대 세 번 써야한다는 것을 알게 됩니다.
추의 무게가 각각 $w_1$, $w_2$, $w_3$이라고 하면, 저울을 사용하는 방법은 다음 두 가지 경우로 정리됩니다.

1. 운이 좋으면 두 번만 재는 것으로 끝납니다.
   예를 들면 $w_1 < w_2$이고, $w_2 < w_3$인 걸 알아낸 경우입니다.

2. 그렇지 않으면 세 번 비교해야 합니다.
   예를 들어 $w_1 < w_2$이고, $w_2 > w_3$이어서, 나머지 $w_2$와 $w_3$의 순서를 알아내야 할 때입니다.

그런데 이렇게 설명하는 대신 트리<L>tree</L>를 그려볼 수도 있습니다.
저울이 항상 두 가지 결과를 가진다고 생각해봅시다.
오른쪽이 내려오거나 그렇지 않은 경우입니다.
그러면 저울로 순서를 결정하는 과정은, 각 노드<L>node</L>가 두 개의 자식 노드를 갖는 바이너리 트리<L>binary tree</L>로 나타낼 수 있게 됩니다.
...
여기서 각 노드는 저울의 결과를 나타내고, 마지막에 나타나는 노드, 즉 리프 노드<L>leaf node</L>는 그런 저울로 결정된 추의 순서를 나타냅니다.
이렇게 결정 과정을 나타내는 트리를 결정 트리 혹은 디시전 트리<L>decision tree</L>라고 부릅니다.

추가 세 개인 경우 디시전 트리를 그리는 것은 어렵지 않지만, 네 개를 넘어가기 시작하면 점점 쉽지 않아집니다.
...
그리고 그 높이가 바로 저울의 사용 횟수를 의미하기 때문에, 이로부터 저울을 적어도 몇 번 써야하는지, 나아가 비교 기반 소팅 알고리즘의 시간 복잡도가 적어도 얼마인지 구하게 됩니다.
...
그리고 [스털링 근사][wp-stirling]<L>Stirling's approximation</L>를 이용하면 $\lg (n!) \sim n \lg n = \Theta(n \lg n)$을 얻습니다.

[wp-stirling]: https://ko.wikipedia.org/wiki/%EC%8A%A4%ED%84%B8%EB%A7%81_%EA%B7%BC%EC%82%AC

물론 이는 디시전 트리가 아니라 바이너리 트리의 특징에서 이끌어낸 결과지만, 어쨌든 디시전 트리 또한 높이가 적어도 $\lg (n!)$이 됩니다.
그러므로 모든 비교 기반 소팅 알고리즘은 최악의 경우에 이 이상의 시간 복잡도, 즉 $\Omega(n \lg n)$을 갖게 됩니다.
다시 말해, $O(n)$ 같은 시간 복잡도를 갖는 그런 알고리즘은 이론적으로 존재할 수 없게 됩니다.

한편, 이론적으로 시간 복잡도의 한계를 구한 것과, 그런 시간 복잡도를 갖는 알고리즘을 생각해내는 것은 별개의 일입니다.
그런데 실제로 최악의 경우에 $\Theta(n \lg n)$의 시간 복잡도를 갖는 비교 기반 소팅 알고리즘으로 머지 소트<L>merge sort</L>가 있습니다.
디시전 트리가 최소 $\Theta(n \lg n)$의 높이를 가진다는 사실을 고려하면, 이는 최적의 시간 복잡도를 가졌다고 말할 수 있습니다.

여기서는 머지 소트를 다른 라이브러리 도움 없이 자바<L>Java</L>로 구현해보고, 두 가지 방법으로 개선을 시도해보겠습니다.
하나는 <PostLink href="/2024/05/16/01">이전 글</PostLink>에서 알아봤던 인서션 소트<L>insertion sort</L>를 섞는 것이고, 또 다른 하나는 내츄럴 머지 소트<L>natural merge sort</L>라는 알고리즘입니다.
...
머지 소트의 아이디어는 이런 문제로 소개할 수 있습니다.
소트된 두 카드 덱이 있다고 해봅시다.
이를 하나의 덱으로 어떻게 소트할 수 있을까요?

<Figure src={fig4} alt="merge example">
  <FigureCaption slot="caption">그림 4. 카드의 머지 예시. 두 덱 중 가장 낮은 카드를 가져와 소트합니다.</FigureCaption>
</Figure>

먼저, 각 맨 앞의 카드가 가장 낮다는 사실을 이용합니다.
그렇다면 둘 중 더 낮은 것이 전체 중에서도 가장 낮은 것입니다.
따라서 그것을 가져와 맨 앞에 놓습니다.
그리고 이를 반복하면, 자연스럽게 하나의 덱으로 합치게 되면서 소트하게 됩니다.
이 과정을 머지<L>merge</L>한다고 부릅니다.

이와 같이, 머지 소트는 이미 소트된 두 데이터셋<L>dataset</L>을 합쳐서, 즉 머지해서 하나로 만듭니다.
그리고 그렇게 하기 위해서, 작은 크기의 데이터셋이 미리 소트되어 있어야 하는데요.
그런데 데이터가 하나밖에 없다면 이미 소트되었다고 볼 수 있습니다.
따라서 머지 소트가 해야할 일은, 먼저 전체를 하나씩 데이터셋으로 나눈 다음, 이로부터 소트된 데이터셋을 합치며 소트하는 것입니다.
...
데이터셋이 배열로 주어진다고 한다면, 이 아이디어를 표현한 수도코드<L>pseudocode</L>는 이렇습니다.
...
<PostLink href="/2024/05/16/01">이전 글</PostLink>처럼 이 알고리즘에서도, 구간을 표현하는 두 인덱스가 반열린구간<L>half-open interval</L> $[\textit{begin}, \textit{end})$, 즉 구간의 시작 $\textit{begin}\,$은 포함하고 마지막 $\textit{end}\,$는 제외한 구간을 나타냅니다.
...
그러다가 데이터셋의 크기가 하나가 되어서야 본격적인 소트를 시작합니다.
그러면 머지를 통해 소트된 부분이 점점 커지면서 소트가 끝납니다.

마지막에 쓰인 머지 알고리즘은 카드 예시에서 봤던 두 덱을 합치는 과정을 수행합니다.
여기선 두 데이터셋이 배열의 구간 $[\textit{begin}, \textit{mid})$, $[\textit{mid}, \textit{end})$로서 주어진다고 합시다.
머지 알고리즘은 두 데이터셋에서 낮은 것부터 가져오며 하나의 데이터셋으로 소트합니다.
이를 수도코드로 표현하면 이렇습니다.

<Alg>
  <AK>머지</AK> ($\textit{arr}$, $\textit{begin}$, $\textit{mid}$, $\textit{end}$) <AC>// 배열 $\textit{arr}\,$에서 두 구간 $[\textit{begin},\textit{mid})$와 $[\textit{mid},\textit{end})$를 머지</AC>
...
반복문에서 더 낮은 데이터를 가져올 때, 두 데이터의 순서가 같은 경우 어떻게 처리하느냐가 중요한 요소입니다.
이는 소팅 알고리즘이 기존 데이터의 상대적인 순서를 유지할지, 즉 스테이블<L>stable</L>할 지를 결정하기 때문입니다.
...
시간 복잡도는 수행한 수도코드 줄의 개수라고 합시다.
머지 알고리즘은 구간의 길이가 $n$이면 반복문을 $n$ 번 수행합니다.
반복문은 네 줄의 수도코드를 수행하고, 반복문 외에는 다섯 줄을 수행하므로, 머지의 시간 복잡도 $T_m(n)$은 $4n + 5$로 쓸 수 있습니다.

그렇다면 머지를 이용하는 머지 소트의 시간 복잡도 $T(n)$을 구해봅시다.
...
한편, $T(\lceil n / 2 \rceil)$와 $T(\lfloor n / 2 \rfloor)$은 구간을 반으로 나눈 재귀 수행 때문이고, $T_m(n)$은 머지로 인한 것입니다.
그렇지 않고 $n \leq 1$이라면, 두 줄의 수도코드가 배열을 바로 리턴하므로 $T(n)=2$가 됩니다.

이렇게 플로어 함수와 실링 함수가 필요한 경우는, 구간의 크기 $n$이 $5$ 처럼 홀수라서 $n/2$에 소수점이 나타날 때입니다.
즉 $\lfloor 2.5 \rfloor = 2$, $\lceil 2.5 \rceil = 3$ 처럼 각각 소수점 버림과 올림으로 처리하기 위해 쓰입니다.
...
따라서 각각을 단순히 $n/2$으로 바꾼 다음 점화식을 풀어도 무방합니다.
...
단지 두 부등식 $T(n) \leq 2T(\lceil n/2 \rceil) + T_m(\lceil n \rceil)$과 $T(n) \geq 2T(\lfloor n/2 \rfloor) + T_m(\lfloor n \rfloor)$에서, 비슷한 증명을 되풀이해 같은 결과를 얻어내면 됩니다.

## 자바로 구현하기

<PostLink href="/2024/05/16/01">이전 글</PostLink>에서는 소팅 알고리즘을 스트레터지 패턴<L>strategy pattern</L>으로 구현하기 위해, 소팅 스트레터지 클래스가 다음과 같은 인터페이스를 만들었습니다.
...
이는 배열 `arr`에서 구간 [`begin`, `end`)을 `comp` 순서로 소트하는 메소드를 의미합니다.

여기서도 머지 소트를 스트레터지로 구현합니다.
앞으로 여러 방법으로 머지 소트를 만들 것이기 때문에, 일단 추상 머지 클래스를 만듭시다.
...
이어서 다음과 같이 `SortStrategy` 인터페이스를 구현하도록 만듭니다.
`sortArray()` 메소드는 머지 결과를 임시로 담을 `aux` 배열을 관리하고, 실제 소트는 `mergeSortArray()` 메소드에 맡깁니다.
...
이렇게 만든 머지 소트 스트레터지는 이하의 테스트 코드처럼 사용할 수 있습니다.
[Junit 5][junit5] 프레임워크를 이용해 작성한 다음 테스트 코드는 `unsorted` 배열이 `expected` 배열로 소트되는지 확인합니다.
여기선 <PostLink href="/2024/05/16/01">이전 글</PostLink>에서 만들어둔 `Sorter` 클래스의 `sortArray()` 메소드를 이용합니다.
...
시간 복잡도를 분석해 얻은 $\Theta(n \lg n)$ 같은 결과는 $n$이 커짐에 따른 경향성을 보여주지만, 반대로 작을 때는 얘기해주지는 않습니다.
예를 들어, 인서션 소트는 최악의 경우에 $\Theta(n^2)$의 시간 복잡도를 가지므로 $n$이 커지면 언젠가는 머지 소트보다 느릴 것이지만, $n$이 작을 때도 그렇다고 단정할 수는 없습니다.

인서션 소트에서의 최선의 경우와 최악의 경우에, 즉 배열이 이미 소트됐을 때와 반대 순서로 됐을 때, 작은 $n$으로 소요 시간을 측정해봅시다.
그러면 $n$이 충분히 작을 땐 인서션 소트가 유리하다는 것을 보여줍니다.
...
배열이 이미 소트된 경우, 인서션 소트의 소요 시간이 더 적습니다. 반대로 소트된 경우, $n = 2^4 = 16$ 부근까지 그렇습니다.
...
계산의 편의를 위해 $n = 2^s$ 꼴이고, $k = 2^t$ 꼴이라고 합시다.
물론 이렇게 해도 일반성을 잃지 않을 것입니다.
앞에서 했던 방법처럼 점화식을 반복 적용하면 다음을 얻습니다.
...
여기서 $k = 1$이면 데이터를 한 개까지 나누는 기존의 머지 소트를 의미하고, 실제로 $T(n)$은 $n \lg n$가 됩니다.
...
따라서 $k$ 값은 낮은 숫자 중에 고르는 것이 낫다는 결론이 나옵니다.
...
이는 이론적인 의미가 있다기보다, 곧 볼 것처럼 실제로 소요 시간에 개선을 보이기 때문에 선택한 값입니다.
...
이렇게 바꾼 소트 또한 여전히 스테이블해야 합니다.
왜냐면 인서션 소트 또한 스테이블하기 때문입니다.
(직접 확인해보세요.)
...
머지 소트가 재귀를 통해 바꾸는 것은 소트 구간이고, 그것이 결국은 하나에서 시작해 두 배 가량 늘어난다는 것을 알고 있습니다.
그러면 재귀를 없애고 대신 반복문이 크기를 조절하도록 바꿔볼 수 있습니다.

예를 들면, 재귀로 구간을 하나씩 나누는 과정 없이, 처음부터 구간의 크기 $\textit{size}_\textit{\small m}$이 하나인 구간을 전부 머지합니다.
그러기 위해 배열의 처음에서부터 두 구간씩 머지합니다.
이를 $[\textit{begin}_\textit{\small m}, \textit{mid}_\textit{\small m})$과 $[\textit{mid}_\textit{\small m}, \textit{end}_\textit{\small m})$라고 합시다.
그 다음 두 배로 늘려 크기가 $\textit{size}_\textit{m}=2$인 구간으로 나누어 머지하고, 이 과정을 반복합니다.
...
하지만 탑 다운 방식은 재귀를 수행할 때마다 이를 위한 스택이 메모리를 사용하지만, 바텀 업 방식은 그런 스택이 필요 없으므로 그만큼 메모리를 덜 쓴다는 장점이 있습니다.
...
이렇게 바텀 업으로 구현한 머지 소트 또한 스테이블합니다.

## 내츄럴 머지 소트

머지 소트는 데이터를 하나씩 나눕니다.
왜냐면 데이터 하나는 이미 소트된 것이기 때문입니다.
그런데 실제 데이터에는 두 개 이상 소트된 부분이 있을 수도 있습니다.
그렇다면 하나씩 나누는 대신, 소트된 부분으로 나누면 소요 시간을 줄일 수 있지 않을까요?

이 아이디어를 이용해, 이미 소트된 부분으로 나누는 방식을 내츄럴 머지 소트<L>natural merge sort</L>라고 부릅니다.
그리고 그 소트된 부분을 런<L>run</L>이라고 부릅니다.
즉, 내츄럴 머지 소트는 런을 머지하는 소트입니다.

<Figure src={fig11} alt="natural merge sort">
  <FigureCaption slot="caption">그림 11. 카드의 내츄럴 머지 소트 예시. 이미 소트된 부분인 런으로 나누고 머지합니다.</FigureCaption>
</Figure>

런을 먼저 조사한 다음에 머지할 수도 있지만, 여기서는 런을 그때그때 조사하며 머지하겠습니다.
남은 런이 두 개 이하라면 머지 후 끝내고, 그렇지 않다면 나머지 런을 모두 머지한 다음 이 과정을 반복합니다.
이를 수도코드로 표현하면 이렇습니다.

<Alg>
  <AK>내츄럴 머지 소트</AK> ($\textit{arr}$, $\textit{begin}$, $\textit{end}$) <AC>// 배열 $\textit{arr}\,$에서 구간 [$\textit{begin},\textit{end}$)의 데이터 소트</AC>

  <ABlock>
    <AWhile postfix=" 항상 반복"></AWhile>
    <ABlock>
      $\textit{begin}_\textit{\small r} \leftarrow$ $\textit{arr}$의 $[\textit{begin}, \textit{end})$에서 첫 두 런 머지 및 다음 런 시작 구함

      <AIf>$\textit{begin}_\textit{\small r} = \textit{end}$ </AIf>&nbsp;<AC>// 소트가 끝났으면</AC>
      <ABlock>
        <AK>반복 중단</AK>
      </ABlock>
      ...
      <AWhile postfix=" 항상 반복"></AWhile>
      <ABlock>
        $\textit{begin}_\textit{\small r} \leftarrow$ $\textit{arr}$의 $[\textit{begin}_\textit{\small r}, \textit{end})$에서 첫 두 런 머지 및 다음 런 시작 구함
...
여기서 구간의 첫 두 런을 머지하는 알고리즘은 다음과 같습니다.

<Alg>
  <AK>첫 두 런 머지 및 다음 런 시작 구하기</AK> ($\textit{arr}$, $\textit{begin}$, $\textit{end}$)

  <ABlock>
    $\textit{end}_{\small 1} \leftarrow$ 구간 $[\textit{begin}, \textit{end})$에서 첫 런의 끝

    <AIf>$\textit{end}_{\small 1} = \textit{end}$ </AIf>&nbsp;<AC>// 두 번째 구간이 없으면 머지 없이 리턴</AC>
    <ABlock>
      <ARet>$\textit{end}_{\small}$</ARet>
    </ABlock>

    <ANL />

    $\textit{end}_{\small 2} \leftarrow$ 구간 $[\textit{end}_{\small 1}, \textit{end})$에서 첫 런의 끝

    머지($\textit{arr}$, $\textit{begin}$, $\textit{end}_{\small 1}$, $\textit{end}_{\small 2}$)

    <ANL />

    <ARet>$\textit{end}_{\small 2}$</ARet>
  </ABlock>
</Alg>

최선의 경우는 이미 소트된 배열이고, 머지 없이 끝나기 때문에, 시간 복잡도는 $\Theta(n)$이 됩니다.
한편, 최악의 경우는 반대 순서로 소트되어서 기존 머지 소트와 같이 데이터를 하나씩 나눌 때입니다.
따라서 기존 머지 소트처럼 $\Theta(n \lg n)$를 갖습니다.

## 자바로 구현하기

수도코드를 그대로 옮겨 자바로 내츄럴 머지 소트 클래스를 구현합니다.
...
또한 두 런을 머지하고 다음 런의 시작을 얻는 `mergeAndGetNextBegin()` 메소드 또한 수도코드를 그대로 옮긴 것입니다.

```java
  private int mergeAndGetNextBegin(T[] arr, int begin, int end,
      Comparator<T> comparator) {
...
다음 런의 끝을 얻는 `getRunEnd()` 메소드는 수도코드에선 한 줄로 표현한 부분입니다.
이는 다음과 같이, 순서대로 있는 구간의 마지막을 리턴하고, 끝을 만나면 끝을 리턴합니다.

```java
  private int getRunEnd(T[] arr, int begin, int end,
      Comparator<T> comparator) {
...
여기서 비교를 맡는 `isGreasterThan()` 메소드는 다음과 같이 유틸로 분리한 것입니다.

```java
public class Comparators {
  // ...

  public static <T> boolean isGreaterThan(T source, T target,
      Comparator<T> comparator) {
    return comparator.compare(source, target) > 0;
  }
}
...
이렇게 만든 내츄럴 머지 소트는 기존 머지 소트와 똑같은 테스트 코드를 통과합니다.
...
내츄럴 머지 소트의 소요 시간을 바텀 업 방식과 비교해보면 특징이 드러납니다.
여기서도 배열이 이미 소트됐을 때와 반대 순서로 소트됐을 때를 봅니다.

<Figure src={fig12} alt="natural vs bottom-up merge sort">
  <FigureCaption slot="caption">그림 12. 바텀 업 방식과 내츄럴 머지 소트의 소요 시간 비교. 선은 회귀선.</FigureCaption>
</Figure>

이미 소트된 배열의 경우, 내츄럴 머지 소트는 머지 과정 없이 끝나기 때문에, 기존 머지 소트보다 소요 시간이 적습니다.
반면 반대로 소트된 배열의 경우, 내츄럴 머지 소트는 런의 크기가 하나라는 것을 알아내기 위해 별도의 과정이 필요하지만, 기존 머지 소트는 그런 오버헤드가 없으므로 그만큼 줄어든 소요 시간을 가집니다.



# 인버전 개수 세기

<PostLink href="/2024/05/16/01">이전 글</PostLink>에서 인버전을 알아봤는데요.
인버전이란 반대 순서로 있는 두 데이터를 말하고, 데이터가 $n$ 개일 때 인버전은 최대 $n(n-1)/2$ 개가 있고, 인서션 소트가 스왑을 수행하는 횟수는 인버전의 개수와 같다는 사실을 소개했습니다.
...
그런데 머지 소트에서도 인버전을 셀 수 있습니다.
두 배열을 머지할 때, 가져올 데이터가 인덱스가 큰 쪽에 있는 경우, 이것은 인덱스가 작은 쪽의 모든 데이터와 인버전을 이룹니다.
반면, 가져올 데이터가 인덱스가 작은 쪽에 있으면 인버전은 없습니다.

<Figure src={fig13} alt="inversions">
  <FigureCaption slot="caption">그림 13. 머지를 통해 인버전을 세는 예시. 가져올 데이터가 인덱스가 큰 쪽에 있으면, 인덱스가 작은 쪽의 데이터와 모두 인버전을 이룹니다. 그렇지 않으면 인버전이 없습니다.</FigureCaption>
</Figure>

이렇게 센 인버전은 머지 구간의 인버전 개수가 됩니다.
그리고 이 구간에서 인버전을 해소하더라도, 이 구간의 데이터와 다른 구간의 데이터가 이루는 인버전은 그대로 남아있습니다.
따라서 머지 소트로 인버전을 해소하면서 센 것이 정확히 모든 인버전 개수가 됩니다.
시간 복잡도는 머지 소트처럼 $\Theta(n \lg n)$을 가집니다.

구현의 예시로, 머지를 맡는 `mergeArrays()` 메소드가 인버전을 세도록 수정해볼 수 있습니다.
다음은 `numInv` 변수로 인버전을 세서 리턴합니다.
...
머지 소트는 이런 `mergeArrays()` 메소드가 리턴하는 개수를 전부 더해 총 인버전 개수를 구할 수 있게 됩니다.
