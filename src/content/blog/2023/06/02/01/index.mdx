...
카탈란 넘버<AltLang>Catalan number</AltLang>라고 불리는 이 숫자들은 다음과 같은 수열, 즉 [시퀀스][wp-seq]<AltLang>sequence</AltLang>입니다.
...
[바이너리 트리][wiki-bin-tree]<AltLang>binary tree</AltLang>의 개수를 세다보면 카탈란 넘버를 마주치게 됩니다.
...
파이썬<AltLang>Python</AltLang>으로 구현하면 다음과 같습니다.
...
다이나믹 프로그래밍<AltLang>dynamic programming</AltLang>으로 중복 계산을 피할 수 있습니다.
...
카탈란 넘버의 클로즈드 폼<AltLang>closed-form</AltLang> 공식 $C_n = \frac{1}{n+1} {2n \choose n}$도 함께요.

카탈란 넘버가 나타나는 문제는 더 다양한데요.
그 중에 [딕 패스][wolfram-dyck-path]<AltLang>Dyck path</AltLang> 문제는 주목해볼만 합니다.
