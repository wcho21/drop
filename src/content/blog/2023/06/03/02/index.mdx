...
여기서는 볼록 다각형<AltLang>convex polygon</AltLang>을 삼각형으로 나눠볼 건데요.
...
$$
여기서 ${2n \choose n}$은 [이항 계수][wiki-bin-coeff]<AltLang>binomial coefficient</AltLang>입니다.
...
이항 계수와 간단한 미적분이 등장하는데 이는 배경 지식으로 전제하고 진행하겠습니다.
...
[스털링 근사][wiki-stirling]<AltLang>Stirling's approximation</AltLang>를 이용하면 이렇게 됩니다.
...
$$
이렇게 [빅오 표기법][wp-big-o]<AltLang>big-O notation</AltLang>으로 카탈란 넘버의 어퍼 바운드<AltLang>upper bound</AltLang>를 알아낼 수 있었습니다.
...
이 증명이 수학적인 확신(?)이 되었길 바라며 카탈란 넘버에 관한 글은 이정도로 정리하겠습니다.
