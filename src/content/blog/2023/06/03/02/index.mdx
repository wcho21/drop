...
[다각형 트라이앵귤레이션][wp-poly-tri]<L>polygon triangulation</L>이라고 불리는 이 문제는 다각형을 삼각형으로 나누는 방법을 세는 것이었습니다.

여기서는 볼록 다각형<L>convex polygon</L>을 삼각형으로 나눠볼 건데요.
...
$$
그리고 이를 이용해 클로즈드 폼<L>closed-form</L>을 얻을 수 있습니다.
$$
...
$$
이렇게 [빅오 표기법][wp-big-o]<L>big-O notation</L>으로 카탈란 넘버의 어퍼 바운드<L>upper bound</L>를 알아낼 수 있었습니다.
