...
import Fleuron from "@components/post/Fleuron.astro";
...
<Figure
  src={fig1}
  alt="Pentagon triangulation"
  caption="오각형 트라이앵귤레이션" />
...
<Figure
  src={fig2}
  alt="Three cases for a given vertex"
  caption="한 모서리에 대한 세 가지 케이스" />
...
<Figure
  src={fig3}
  alt="General cases for a given vertex"
  caption="다각형을 나누는 삼각형" />
...
<Figure
  src={fig5}
  alt="Duplicate cases"
  caption="하나의 트라이앵귤레이션에 대한 중복 케이스들" />
...
[테일러 전개][wp-taylor]를 이용할 때, $\sqrt{1-4x} = 1 - 2x + \cdots$ 로부터, $\frac{1 + \sqrt{1 - 4x}}{2x} = \frac{1}{x} - 1 + \cdots$ 이므로 $\frac{1}{x}$이 나와 맞지 않기 때문입니다.
...
카탈란 넘버가 나타나는 다양한 문제는 *Catalan Numbers* (Richard Stanley, 2015)에서 찾아볼 수 있습니다.
카탈란 넘버의 점화식과 클로즈드 폼의 증명은 *Catalan Numbers with Application* (Thomas Koshy, 2008)에서 참고할 수 있습니다.
