...
\begin{document}
  \begin{tikzpicture}
    ...
    \draw[{Stealth[scale=\arrowtipscale]}-] (P14) .. controls +(60:3mm) and +(left:3mm) .. ++(12mm,6mm) coordinate (R);
    \node[right] at (R) {\small $k$-gon};

    % left annotation
    \path (P6) -- (P8) coordinate[midway] (P68);
    \draw[{Stealth[scale=\arrowtipscale]}-] (P68) .. controls +(135:12mm) and +(90:6mm) .. ++(-9mm,-9mm) coordinate (L);
