...
<PostLink href="/2023/06/02/01">이전 글</PostLink>에서 다음과 같이 정의된 카탈란 넘버를 간단히 알아보았는데요.
$$
C_n = C_0 C_{n-1} + C_1 C_{n-2} + \cdots + C_{n-1} C_0
$$
카탈란 넘버는 다양한 문제에서 나타납니다.
대표적으로 바이너리 트리의 개수를 세는 문제를 살펴봤습니다.

이것 말고도 다른 문제도 많은데요.
여기서는 문제를 직접 해결하는 대신, 이미 해결한 문제로 환원<AltLang>reduce</AltLang>하는 방법으로 풀어볼 것입니다.
어렵게 말하면 두 문제 사이에 일대일 대응, 즉 [바이젝션][wiki-bijection]<AltLang>bijection</AltLang>을 찾는 것입니다.

[wiki-bijection]: https://en.wikipedia.org/wiki/Bijection

우선 카탈란 넘버가 나타나는 또 다른 대표적인 문제인 [딕 패스][wolfram-dyck-path]<AltLang>Dyck path</AltLang> 문제를 보겠습니다.
...
여태까지 살펴본 문제는 모두 카탈란 넘버가 답이었는데요.
이상하리만큼 다양한 곳에서 등장하는 숫자였습니다.

사실 역사를 거슬러 올라가보면 오일러<AltLang>Leonhard Euler</AltLang>와 함께 트라이앵귤레이션<AltLang>triangulation</AltLang>이라는, 다각형을 삼각형으로 나누는 방법의 개수를 구하는 문제가 등장하는데요.
여기서 카탈란 넘버의 점화식 대신 클로즈드 폼<AltLang>closed-form</AltLang> 공식인 $C_n = \frac{1}{n+1}{2n \choose n}$을 얻을 수 있습니다.
