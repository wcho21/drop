...
이것 말고도 다른 문제도 많은데요.
여기서는 문제를 직접 해결하는 대신, 이미 해결한 문제로 환원<L>reduce</L>하는 방법으로 풀어볼 것입니다.
어렵게 말하면 두 문제 사이에 일대일 대응, 즉 [바이젝션][wiki-bijection]<L>bijection</L>을 찾는 것입니다.
...
사실 역사를 거슬러 올라가보면 오일러<L>Leonhard Euler</L>와 함께 트라이앵귤레이션<L>triangulation</L>이라는, 다각형을 삼각형으로 나누는 방법의 개수를 구하는 문제가 등장하는데요.
여기서 카탈란 넘버의 점화식 대신 클로즈드 폼<L>closed-form</L> 공식인 $C_n = \frac{1}{n+1}{2n \choose n}$을 얻을 수 있습니다.
