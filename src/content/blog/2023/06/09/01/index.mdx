...
summary: "코드 포인트에서 시작하는 UTF-8 인코딩"
...
import fig7 from "./_figs/fig7.svg";

UTF-8은 문자 인코딩에서 자주 마주치게 되는 용어인데요.
벨<L>Bell</L> 연구소에서 유닉스<L>Unix</L>의 성공에 이어 발표한 Plan 9이란 운영체제는 당시 리눅스에 밀리고 말았지만, UTF-8이라는 유명한 유산을 남기고 갔습니다.
그리고 이것은 2023년에 이르러 웹에서 [약 98%의 점유율][w3tech-utf-8]을 가질 정도로 퍼지게 되었습니다.

그런데 이 UTF-8 인코딩은 어떤 방식으로 문자를 표현하고 있을까요?
다른 인코딩 방식에 비해 나은 점은 뭘까요?
그리고 유니코드<L>Unicode</L>와는 어떤 관계가 있을까요?

이 글에서는 UTF-8를 직접 구현해보며 자세히 알아보겠습니다.
이를 통해 유니코드 상 $2^{21}$개에 달하는 문자, 즉 코드 포인트<L>code point</L>를 UTF-8로 완전하게 커버할 수 있음을 보게 됩니다.
...
추상적인 한 심볼<L>symbol</L>로서 떠올려봅시다.

유니코드는 이런 추상적인 심볼에서 비트 표현에 이르기까지 [여러 단계로 구분][utr-17-model]하고 있는데요.
여기서는 대신 간단히 두 단계만 매핑<L>mapping</L>으로서 언급해보겠습니다.
...
하나는 문자를 코드 포인트<L>code point</L>라고 부르는 숫자에 대응시키는 매핑인데요.
유니코드란 이러한 매핑입니다.

예를 들어, 유니코드는 그리스 문자 'α'에 945번이라는 코드 포인트를 매겨놓습니다.
이는 16진수로 3B1번이 되기 때문에 `U+03B1`로 표기합니다.
뒤에서 보겠지만, 이것은 유니코드가 가질 수 있는 $2^{21}$개의 코드 포인트 중 하나입니다.

또 다른 하나는 이 코드 포인트에서 구체적인 메모리 상의 비트 표현, 즉 코드 유닛 시퀀스<L>code unit sequence</L>로 대응시키는 매핑입니다.
UTF-8은 이러한 매핑 중 하나입니다.
이름에서 8은 코드 유닛 시퀀스가 8비트임을 의미하는데요.
즉 8비트 단위로 시퀀스를 나열하기 때문에, 인코딩된 결과는 8비트, 16비트, 24비트와 같이 8비트의 배수만큼 길어집니다.
구체적인 매핑 방법은 직접 구현할 때 보겠습니다.

정리하면 유니코드는 문자와 코드 포인트 사이의 매핑이고, UTF 인코딩은 코드 포인트와 비트 표현 사이의 매핑입니다.
이런식으로 문자는 하나의 비트 표현을 갖게 됩니다.



# 직접 구현해보는 UTF-8 인코딩

이제 UTF-8 인코딩을 이용해 문자를 비트 표현으로 바꿔봅시다.
문자와 코드 포인트 간의 관계는 이미 유니코드에 정해져있습니다.
따라서 코드 포인트가 주어졌을 때 그것을 바이트 시퀀스<L>byte sequence</L>, 즉 바이트의 나열로 바꾸는 함수를 구현하면 됩니다.
이를 인코딩<L>encoding</L>이라고 부를 수 있는데요.
UTF-8로 인코딩하는 방법 또한 이미 정해져있기 때문에 그것을 구현하기만 하면 됩니다.

## UTF-8 인코딩

그 인코딩 방법은 무엇일까요?
코드 포인트의 이진 표현<L>binary representation</L>이 몇 비트를 차지하느냐에 따라 케이스를 나누는데요.
...
예를 들어 그리스 문자 'α'의 코드 포인트는 945인데요.
이는 이진 표현으로 `011 1011 0001`이 되기 때문에 두 개의 바이트로 나눠 담고,
인코딩 결과는 `1100 1110 1011 0001`, 즉 16진수로 `CE B1`이 됩니다.
...
예를 들어, `110`으로 시작하는 바이트는, 그 뒤에 `10`으로 시작하는 바이트가 두 개 따라온다는 의미를 가지게 됩니다.
...
아래 수도코드<L>pseudocode</L>의 `encodeUtf8()` 함수는 코드 포인트를 나타내는 숫자를 입력받고 그것을 UTF-8로 인코딩한 바이트 시퀀스를 리턴합니다.
...
`U+FFFD`로 정의된 이 문자는 [대체 문자][wiki-repl-char]<L>replacement character</L>라고 부르는데요.
...
수도코드는 바이트 시퀀스를 리턴했지만, 여기서는 매개변수 `buf`에 사이드 이펙트로 리턴값을 전달합니다.
리턴값 끝에 붙인 널 문자 `\0`는 나중에 출력의 편의를 위해 임의로 붙인 것으로, UTF-8 인코딩과는 무관합니다.
...
터미널이 바이트 시퀀스를 이해하고 화면에 렌더하는 것은 별개의 일입니다.
먼저, 폰트가 그 코드 포인트에 해당하는 문자를 지원하지 않을 수 있습니다.
이 경우 유니코드를 지원하는 폰트를 설치하는 것으로 해결할 수 있습니다.
아니면, 터미널 자체가 인코딩 결과를 이해하지 못할 수도 있습니다.
리눅스<L>Linux</L>나 맥<L>Mac</L>에서는 보통 터미널이 UTF-8 인코딩을 네이티브하게 지원할 것이고요.
윈도우<L>Windows</L>는 다음과 같은 API 호출이 필요할 것입니다.
...
이는 방금 만든 `encodeUtf8()` 함수의 역함수<L>inverse function</L>로서 정반대의 작업을 할 것입니다.
...
여기서 `XX`는 `0`부터 `10`까지 있을 수 있는데요.
이는 곧 플레인의 번호를 가리키게 됩니다.
예를 들어, `U+2345`는 `0`번 플레인, `U+12345`는 `1`번 플레인에 있는 코드 포인트가 됩니다.
...
이런 것이 주제는 아니기 때문에 이 정도만 언급하는 선에서 정리하고 넘어가겠습니다.
...
바이트의 개수는 같지만, 내부적으로는 각각 다른 개수의 코드 유닛으로 나누어 인코딩 됩니다.
...
이 때 나타났던 대표적인 현상으로 소위 '占쏙옙'이 있었습니다.
...
EUC-KR 에서 '占쏙옙'으로 디코딩 되는 코드 유닛 시퀀스는 16진수로 `EF BF BD EF BF BD` 인데요.
한글이 2바이트씩 인코딩되기 때문에 총 6바이트의 크기를 가집니다.
한편 이는 UTF-8 인코딩에서, 앞서 소개한 대체 문자 &lsquo;&#65533;&rsquo;를 연달아 두 개 쓴것과 같은 비트 표현을 가집니다.
...
따라서 UTF-8 인코딩에 실패해 대체 문자로 바뀐 데이터를 EUC-KR로 읽었을 때 나타나는 현상으로 이해할 수 있습니다.


# 그 외의 것들

한편 본문에서 다루지 않았던 내용을 짤막하게 언급하고 넘어가자면 이렇습니다.

## 문자열 길이

가변 폭 인코딩인 UTF-8는 바이트의 개수가 곧 문자의 개수와 일치하지는 않는데요.
하지만 UTF-8로 인코딩된 바이트 시퀀스를 읽어서 문자의 개수를 세는 `strlen()`과 같은 함수를 만들어볼 수 있습니다.
하지만 바이트의 개수를 리턴할 수 없고, 인코딩된 비트를 전부 파악해야 합니다.
...
그런데 코드 포인트 순서가 반드시 문자의 순서라고 말할 수는 없습니다.
예를 들어, 문자 '0'이 문자 'A'보다 먼저일 필요는 없는데요.
따라서 문자 순서를 따로 매길 필요가 있습니다.
이를 콜레이션<L>collation</L>이라고 부릅니다.
특히 데이터베이스에서 이런 콜레이션이 직접적으로 쓰일 수 있습니다.

## 노말라이즈

유니코드 상에서 어떤 문자들은 표현 방법이 하나만 있는 것은 아닌데요.
예를 들어 악센트가 있는 A 문자 '&#192;' (`U+00C0`)를 봅시다.
이 문자는 두 문자를 조합해서 만들 수 있습니다.
문자 'A' (`U+0041`)에 악센트 조합 문자인 '◌̀' (`U+0300`)를 더해서 '&#65;&#768;'로요.
(방금 실제로 두 문자를 조합했습니다.)

정규화<L>normalization</L>는 이런 문제를 위해 문자를 하나로 통일시키는 작업을 일컫습니다.
대표적으로 예를 들면 자바스크립트의 메소드인 [`normalize()`][mdn-normalize]가 그 정규화를 맡고 있습니다.
그래서 특별한 경우에는 두 문자열의 비교 전에 먼저 노말라이즈를 해야할 필요가 생깁니다.
그리고 이는 UTF-8 인코딩이 아닌 유니코드가 가진 특징이기 때문에, UTF-16과 UTF-32 또한 똑같은 문제를 가지게 됩니다.
