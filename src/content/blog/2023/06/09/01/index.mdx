...
벨<AltLang>Bell</AltLang> 연구소에서 유닉스<AltLang>Unix</AltLang>의 성공에 이어 발표한 Plan 9이란 운영체제는 당시 리눅스에 밀리고 말았지만, UTF-8이라는 유명한 유산을 남기고 갔습니다.
...
그리고 유니코드<AltLang>Unicode</AltLang>와는 어떤 관계가 있을까요?

이 글에서는 UTF-8를 직접 구현해보며 자세히 알아보겠습니다.
이를 통해 유니코드 상 $2^{21}$개에 달하는 문자, 즉 코드 포인트<AltLang>code point</AltLang>를 UTF-8로 완전하게 커버할 수 있음을 보게 됩니다.
...
추상적인 한 심볼<AltLang>symbol</AltLang>로서 떠올려봅시다.

유니코드는 이런 추상적인 심볼에서 비트 표현에 이르기까지 [여러 단계로 구분][utr-17-model]하고 있는데요.
여기서는 대신 간단히 두 단계만 매핑<AltLang>mapping</AltLang>으로서 언급해보겠습니다.
...
하나는 문자를 코드 포인트<AltLang>code point</AltLang>라고 부르는 숫자에 대응시키는 매핑인데요.
유니코드란 이러한 매핑입니다.
...
또 다른 하나는 이 코드 포인트에서 구체적인 메모리 상의 비트 표현, 즉 코드 유닛 시퀀스<AltLang>code unit sequence</AltLang>로 대응시키는 매핑입니다.
...
이를 인코딩<AltLang>encoding</AltLang>이라고 부를 수 있는데요.
...
리눅스<AltLang>Linux</AltLang>나 맥<AltLang>Mac</AltLang>에서는 보통 터미널이 UTF-8 인코딩을 네이티브하게 지원할 것이고요.
윈도우<AltLang>Windows</AltLang>는 다음과 같은 API 호출이 필요할 것입니다.
...
이는 방금 만든 `encodeUtf8()` 함수의 역함수<AltLang>inverse function</AltLang>로서 정반대의 작업을 할 것입니다.
...
이를 각각 플레인<AltLang>plane</AltLang>이라고 부릅니다.
...
이 플레인을 다국어 기본 평면<AltLang>Basic Multilingual Plane (BMP)</AltLang>이라고 부르는데요.
...
문자마다 코드 유닛의 개수, 즉 바이트의 개수가 달라지므로, UTF-8은 가변 폭 인코딩<AltLang>variable-width encoding</AltLang>이라고 부릅니다.
