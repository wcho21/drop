...
\begin{document}
  \begin{tikzpicture}
    ...
    \draw[{Stealth[scale=1.2]}-] (2.75*\side,3.75*\side) .. controls +(60:8mm) and +(120:8mm) .. ++(\altxoffset-2.75*\side+\altside,0) node[below] {Plane};
    ...
    \draw[{Stealth[scale=1.2]}-] (\altxoffset+0.5*\altside,\altyoffset-2*\altside+0.25*\altside) .. controls +(-120:3mm) and +(120:2mm) .. ++(-2mm,-8mm) node[below] {Block};
  \end{tikzpicture}
\end{document}
