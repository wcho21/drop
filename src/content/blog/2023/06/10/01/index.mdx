...
summary: "코드 유닛과 UTF-16, UTF-32 인코딩"
...
<PostLink href="/2023/06/09/01">이전 글</PostLink>에서 유니코드<AltLang>Unicode</AltLang>와 UTF-8을 살펴봤습니다.
유니코드는 추상적인 문자와 코드 포인트<AltLang>code point</AltLang> 사이의 매핑으로, UTF-8는 코드 포인트와 코드 유닛 시퀀스<AltLang>code unit sequence</AltLang> 사이의 매핑으로 볼 수 있었습니다.
또한 유니코드는 내부적으로 플레인<AltLang>plane</AltLang> 단위로 코드 포인트를 관리합니다.
...
첫 번째 플레인 BMP에는 서로게이트<AltLang>surrogate</AltLang> 블록이라고 불리는 코드 포인트가 있는데요.
...
구체적으로 어떻게 사용되는 지는 직접 구현을 통해 살펴보겠습니다.


# UTF-16 인코딩 만들기

이전 내용을 정리하자면, 문자와 코드 포인트 간의 대응 관계는 유니코드가 먼저 정의하고 있습니다.
그리고 코드 포인트를 비트 표현으로 매핑하는 방법을 인코딩이라고 부르기로 했는데요.

그러면 UTF-16는 어떻게 코드 포인트를 인코딩할까요?

코드 포인트를 16비트로 표현할 수 있다면, 그 코드 포인트의 이진 표현<AltLang>binary representation</AltLang>을 그대로 하나의 코드 유닛으로 인코딩합니다.
그렇지 않고 16비트를 넘어간다면, UTF-8에서 여러 개의 코드 유닛에 담았던 것과 같은 방식으로 이루어집니다.
...
두 번째 경우, 코드 포인트의 21비트를 적절히 세 부분으로 나누어 두 개의 코드 유닛에 담아 인코딩하는데요.
이 두 코드 유닛을 서로게이트 페어<AltLang>surrogate pair</AltLang>라고도 부릅니다.
그리고 첫 번째 코드 유닛을 하이 서로게이트<AltLang>high surrogate</AltLang>, 두 번째 것을 로우 서로게이트<AltLang>low surrogate</AltLang>라고 합니다.
...
그리고 리턴값 끝에 붙인 널 문자 `\0`는 출력의 편의를 위한 것으로, UTF-16 인코딩과는 무관합니다.

반대로 UTF-16 인코딩 결과로부터 코드 포인트를 얻는 디코딩 함수는, UTF-8 디코딩과 크게 다르지 않습니다.
이는 직접 해보는 문제로 남겨두고 넘어가겠습니다.
...
한번 생각해볼 수 있는 문제로, 한글 문자를 표현할 때는 UTF-16과 UTF-8 중에 어느 것이 메모리에 더 효율적일까요?
...
물론 그 차이가 의미가 있는지, 그리고 다른 언어가 함께 사용할 때 어떨지는 다른 문제이지만요.


# UTF-32의 구현

UTF-32는 구현하기가 비교적 간단한데요.
...
반대로 코드 유닛을 받아 코드 포인트를 구하는 디코딩 함수는 직접 해보는 문제로 남겨두겠습니다.

여기까지 모든 코드는 디코딩 함수 구현을 포함해 [지스트][gist]에서 확인할 수 있습니다.
...
UTF-32 인코딩 또한 그렇습니다.
이전 글에서 봤던 노말라이즈<AltLang>normalize</AltLang> 문제처럼, 하나의 문자가 두 개의 코드 포인트로 표현될 수 있는 경우가 그 원인이 될 수 있습니다.
...
특히 UTF-16 인코딩에서 서로게이트 블록의 역할을 포함해서요.

한편 인코딩에서 바이트 오더<AltLang>byte order</AltLang>, 즉 [엔디안][wp-endian]<AltLang>endianness</AltLang> 문제가 있는데요.
사실 UTF-16 인코딩은 각 엔디안을 위한 UTF-16LE, UTF-16BE 인코딩이 따로 존재합니다.
아니면 [바이트 오더 마크][wp-bom]<AltLang>byte order mark</AltLang>, 즉 BOM 문자를 맨 앞에 둬서 바이트 순서를 나타내는 방법도 있습니다.
