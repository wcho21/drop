...
코드 포인트를 16비트로 표현할 수 있다면, 그 코드 포인트의 이진 표현<L>binary representation</L>을 그대로 하나의 코드 유닛으로 인코딩합니다.
...
그리고 첫 번째 코드 유닛을 하이 서로게이트<L>high surrogate</L>, 두 번째 것을 로우 서로게이트<L>low surrogate</L>라고 합니다.
...
한편 인코딩에서 바이트 오더<L>byte order</L>, 즉 [엔디안][wp-endian]<L>endianness</L> 문제가 있는데요.
