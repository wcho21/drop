---
title: "부동소수점 비교 연산 구현해보기"
date: 2023-05-19T21:00:00+09:00
summary: "IEEE754 구현을 통해 살펴보는 부동소수점"
thumbnail: "./_figs/thumbnail.webp"
---

import AltLang from "@/components/post/AltLang.astro";
import FigureV2 from "@/components/post/FigureV2.astro";
import FigureCaption from "@/components/post/FigureCaption.astro";

import fig1 from "./_figs/fig1.svg";
import fig2 from "./_figs/fig2.svg";
import fig3 from "./_figs/fig3.svg";

프로그래밍을 하다보면 `0.1 + 0.2`와 `0.3`이 같지 않다는 얘기를 들어보게 됩니다.
자바스크립트 런타임인 노드<AltLang>Node.js</AltLang>에서 확인해볼 수 있는데요.

```
$ node
> 0.1 + 0.2 === 0.3
false
```

파이썬<AltLang>Python</AltLang>에서도 확인해볼 수 있고요.

```
$ python
>>> 0.1 + 0.2 == 0.3
False
```



아마 유한한 자릿수의 비트로는 `0.1`을 정확히 표현할 수 없어서 그런 것 같습니다.

그러면 `0.2`도 그럴까요? `0.3`은 어떨까요?
모든 숫자는 정확하게 표현할 수 없는 걸까요?
그리고 이런 정확도와 비교 연산자는 무슨 관련이 있을까요?

사실 어떤 숫자들은 정확하게 표현할 수 있는데요.
`0.1 + 0.2 == 0.3` 문제를 들여다보기 전에, 먼저 소수점은 어떻게 비트로 표현되는지, 그리고 비교 연산자는 그런 비트를 어떻게 비교하는지 살펴볼 필요가 있습니다.
직접 부동소수점 타입을 구현해보며 이해해봅시다.



# 숫자 해부하기

`0.1`과 같은 숫자를 비트<AltLang>bit</AltLang>로 표현하기 위해, 우리가 흔히 쓰는 하드웨어나 프로그래밍 언어는 [IEEE 754][wp-ieee754]라는 표준을 참고해서 부동소수점<AltLang>floating-point</AltLang>이라는 것을 구현하고 있습니다.

[wp-ieee754]: https://ko.wikipedia.org/wiki/IEEE_754

그러면 이에 따라 어떻게 숫자가 비트로 표현되는지 보겠습니다.

## 소수점 옮기기

숫자 $102.3$을 예로 들어봅시다.
그런데 그 전에, 이 숫자를 다르게 말해봅시다. $1.023 \times 10^2$로요.
[과학적 표기법][wp-sci]<AltLang>scientific notation</AltLang>이라고 불리는 이 방식은 숫자 하나를 항상 소수점 앞에 둡니다.

[wp-sci]: https://ko.wikipedia.org/wiki/%EA%B3%BC%ED%95%99%EC%A0%81_%EA%B8%B0%EC%88%98%EB%B2%95

부동소수점 비트 표현에선 이 10의 지수가 얼마인지가 중요한데요.
여기서는 2가 됩니다.
이렇게 과학적 표기법을 기준으로 말하면 이 지수가 유일하게 결정됩니다.
이를 나중에 노말라이즈<AltLang>normalize</AltLang>한다고 부릅니다.

십진법을 쓰면 맨 앞 숫자는 $9$까지 나올 수 있지만, 이진법을 쓴다면 $1$밖에 올 수 없습니다.
그렇기 때문에 이진법을 쓰면 맨 앞 숫자를 따로 기억할 필요가 없어집니다.
이렇게 비트 공간을 하나 벌게 됩니다.

## 세 부분으로 해부하기

32비트 공간이 주어졌다고 해봅시다.
세 부분으로 나눠볼건데요.
부호<AltLang>sign</AltLang> $S$, 지수<AltLang>exponent</AltLang> $E$, 그리고 소수<AltLang>fraction</AltLang> $F$ 입니다.

<FigureV2 src={fig1} alt="Memory diagram">
  <FigureCaption slot="caption">그림 1. 메모리 다이어그램. 32비트를 세 부분으로 나누어 부호, 지수, 소수를 기록합니다.</FigureCaption>
</FigureV2>

방금의 예시 $1.023 \times 10^2$을 해부해서 여기에 넣어봅시다.

부호는 양수이거나 음수이거나 둘 중 하나기 때문에, 부호 $S$는 비트 하나만 필요한데요.
여기서는 양수라는 뜻으로 $S=0$ 이라고 둡시다.

지수는 $10^2$의 지수자리를 담을 건데요.
따라서 $E=2$로 둡시다.
마지막으로 소수 부분은 $F=1.023$로 넣고요.

정리하면 결과는 이렇습니다.

$$
\begin{align*}
  S &= 0 \\
  E &= 2 \\
  F &= 1.023
\end{align*}
$$

이제 저장한 숫자를 알아내고 싶다면, $(-1)^S \times F \times 10^E$를 계산해보면 됩니다.
그러면 원래 숫자인 $102.3$이 될 것입니다. (확인해보세요.)



# 소수점을 비트로 표현하기

세 부분으로 기억한다고는 했지만, 그래서 구체적으로 어떻게 비트로 기록하는지는 얘기하지 않았는데요.

일단 부호 $S$는 $0$과 $1$ 뿐이고, 이대로 비트에 기록하면 되기 때문에 간단합니다.
나머지는 어떻게 기록하는지 봅시다.

## 이진수로 바꾸기

십진수를 비트로 담기 위해 이진수로 바꾸려는데요.
이는 단순하게 이진법의 정의를 따르면 됩니다.

그런데 그 정의를 보세요.
숫자를 $2^n$ 꼴의 유한한 합으로 표현할 수 없다면, 무한히 많은 자릿수가 필요하게 됩니다.

$$
(\dots b_1 b_0 . b_{-1} \dots)_{(2)} = \sum_n b_n 2^n \quad (b_n = 0, 1)
$$

따라서 $0.5_{(10)}$는 부동소수점으로 정확하게 표현할 수 있지만, $0.1_{(10)}$은 그렇지 않게 됩니다.
$0.0001\overline{1001}_{(2)}$와 같이 순환소수가 되니까요.

## 비트로 인코딩하기

메모리의 소수 부분 $F$는 23비트를 갖고 있는데요.

<FigureV2 src={fig2} alt="Fraction part">
  <FigureCaption slot="caption">그림 2. 메모리 소수 부분. 23자리의 이진법 소수를 기록합니다.</FigureCaption>
</FigureV2>

맨 앞 숫자는 항상 $1$이라고 했던 것을 기억하시나요?
따라서 24자리의 이진수를 기록할 수 있게 됩니다.
그런데 모든 십진수를 24자리의 이진수로 정확하게 바꿀 수 없으니, 적당한 근사값을 여기에 담게 됩니다.

그런 근사값은 올리기를 할 수도, 버리기를 할 수도 있습니다.
사실 라운딩 룰<AltLang>rounding rules</AltLang>이라는 이름으로 다양한 방법이 존재합니다.

## 지수의 바이어스

보통 정수는 [2의 보수][wp-twos-compl]<AltLang>two's complement</AltLang>로 인코딩하는 경우가 많은데요.
여기서는 숫자 $0$을 위해 모든 비트를 `0`으로 둡니다.

[wp-twos-compl]: https://ko.wikipedia.org/wiki/2%EC%9D%98_%EB%B3%B4%EC%88%98

그런데 다른 방법도 있는데요.
가장 작은 숫자을 위해 모든 비트를 `0`으로 둘 수 있습니다.
우리의 지수 부분은 8비트이므로, $-127$이 그 가장 작은 숫자에 해당하는데요.
이를 모든 비트가 `0`인 것에 대응시킬 수 있습니다.
이때 지수가 $127$만큼 바이어스<AltLang>biased</AltLang> 되어있다고 부릅니다.

$$
\begin{align*}
  -127: \quad &\texttt{0 ... 000} \quad (E=0) \\
  -126: \quad &\texttt{0 ... 001} \quad (E=1) \\
  &\vdots \\
  128: \quad &\texttt{1 ... 111} \quad (E=255)
\end{align*}
$$

여기까지 정리하면, 계산식은 $(-1)^S \times (1+F) \times 10^{(E-127)}$이 됩니다.

이렇게 $127$만큼의 차이를 두면 빠른 비교연산을 만들 수 있습니다.
두 지수 부분 비트를 차례대로 읽기만하면 되는데요.
먼저 `1`이 나타나는 쪽이 큰 수가 됩니다.

## 직접 확인해보기

예를 들어 $-2.75_{(10)}$를 바꿔보며 정리해봅시다.

이진수로 고치고 노말라이즈하면 $-1.011_{(2)} \times 2^{1}$가 됩니다.
이로부터 세 부분을 얻을 수 있습니다.

$$
\begin{align*}
  S &= 0 \\
  E &= 128_{(10)} \rightarrow 100 \dots 0_{(2)} \\
  F &= .011_{(2)}
\end{align*}
$$

이제 부동소수점을 위한 32비트 메모리는 다음과 같이 채워져 있을 것입니다.

<FigureV2 src={fig3} alt="Memory diagram example">
  <FigureCaption slot="caption">그림 3. '-2.75'를 기록한 메모리 다이어그램. 맨 앞 1비트 부호 부분에는 음수의 의미로 1을, 8비트 지수 부분에는 바이어스된 값을, 23비트 소수 부분에는 이진법으로 기록합니다.</FigureCaption>
</FigureV2>

정말 그럴까요?
C++ 코드로 아래처럼 확인해볼 수 있습니다.

```cpp
float f = -2.75;
std::cout << std::bitset<32>(*reinterpret_cast<int *>(&f)) << '\n';
// output: 11000000001100000000000000000000
```


# 비교 연산 만들기

나눴던 세 부분의 순서를 보기 위해 다시 그림 1을 가져와봅시다.

<FigureV2 src={fig1} alt="Memory diagram">
  <FigureCaption slot="caption">그림 1. 메모리 다이어그램</FigureCaption>
</FigureV2>

이 순서가 주는 장점이 있습니다.
이 순서대로만 비교하면 빨리 결과를 낼 수 있기 때문입니다.

왜일까요? 부호 부분 $S$가 다르면 나머지는 볼 필요가 없습니다.
지수 부분 $E$가 다르면 소수 부분 $F$은 볼 필요가 없습니다.

예를 들어 $1.x \times 10^2$와 $1.y \times 10^3$이 있다고 해봅시다.
$x$와 $y$가 무엇이든, $1.y \times 10^3$이 더 큰 수가 됩니다.
어차피 소수점 앞은 $1$이니까요.

이렇게 쉽게 비교할 수 있는 이유는, 어떤 숫자든지 소수점 앞에 항상 $1$이 오기 때문입니다.
그렇죠?

사실 아닙니다.
$0$은 그런식으로 표현할 수가 없습니다.

그래서 $0$을 위한 특별한 방법으로, 지수와 소수 부분을 모두 `0`으로 채워넣습니다.
그런데 부호 비트는 여전히 `0`이거나 `1`일 수 있는데요.
따라서 부동소수점이 $0$을 표현하는 방법은 두 가지가 됩니다.

$$
\begin{align*}
  +0: \quad &\texttt{0\,00000000\,000\dots0} \\
  -0: \quad &\texttt{1\,00000000\,000\dots0}
\end{align*}
$$

따라서 비교 연산을 만들 때 이런 부분을 고려해야 합니다.
여기서는 두 가지의 $0$을 같은 수로 처리해봅시다.

여기 두 수가 같은지 비교하는 수도코드입니다.
즉 부동소수점을 위한 `==` 연산으로 볼 수 있습니다.

```
function equal(a, b)
  if a is zero && b is zero
    return true
  if a.sign != b.sign
    return false
  if a.exponent != b.exponent
    return false
  if a.fraction != b.fraction
    return false
  return true
```

## 구현 예시: 파이썬

먼저 부동소수점 타입을 준비해봅시다.
부호와 지수, 소수 부분 필드를 만들고요.

```python
class Float:
    def __init__(self, sign, exp, frac):
        self.sign = sign
        self.exp = exp
        self.frac = frac
```

그리고 `Float` 클래스에 `==` 연산을 오버로딩 해봅시다.
이 부분은 수도코드의 구현입니다.

```python
    def __eq__(self, other):
        if self.isZero() and other.isZero():
            return True
        if self.sign != other.sign:
            return False
        if self.exp != other.exp:
            return False
        if self.frac != other.frac:
            return False
        return True
```

여기서 쓰인 보조 메소드 `isZero()`는 지수와 소수가 둘 다 `0`인지 검사하는 것으로 만듭니다.

```python
    def isZero(self):
        return self.exp == 0 and self.frac == 0
```

이제 테스트해봅시다.

```python
positive_zero = Float(0, 0, 0) # +0
negative_zero = Float(1, 0, 0) # -0
assert positive_zero == negative_zero

two_point_seven_five = Float(0, 1 << 7, 0b011 << 20) # -2.75
assert two_point_seven_five != positive_zero
```

위 코드는 [지스트][gist]에서 확인할 수 있습니다.

[gist]: https://gist.github.com/wcho21/e216323133a8de85adc6a25c3644a0af



# 마치며

여기까지 부동소수점 타입과 비교 연산을 구현해봤습니다.

하지만 `0.1 + 0.2 == 0.3` 문제는 다루지 못했는데요.
그러면 두 가지를 더 만들어야 합니다.
하나는 부동소수점 리터럴, 즉 문자열을 숫자로 바꾸는 것이고, 또 하나는 덧셈 연산을 구현하는 것입니다.
하지만 둘 다 쉬운 일은 아닌데요.
근사값을 구하는 규칙이 따로 필요하고요.
리터럴로부터 파싱하는 방법 또한 필요합니다.

그럼에도 비교 연산으로부터 알 수 있는 게 있습니다.
`0.1 + 0.2`와 `0.3`은 부호와 지수는 같을 것입니다.
즉 소수 부분이 비교를 결정할 것입니다.
