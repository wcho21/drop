...
이를 구현하는 한 가지 방법으로 스택<L>Stack</L>을 이용해 계산기를 만들 수 있습니다.
...
후위 표기식<L>postfix expression</L>이라고 불리는 이것은 복잡해 보일 수 있지만, 곧 만나볼 이유 때문에 계산하기가 쉬워집니다.
...
그런데 사람은 `1+2` 같은 식, 즉 중위 표기식<L>infix expression</L>을 쓰는 것이 익숙할 텐데요.
...
완성한 프로그램은 [지스트][gist]<L>Gist</L>에서 볼 수 있습니다.
