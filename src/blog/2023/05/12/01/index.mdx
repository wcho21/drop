...
`2+3` 대신 `23+`을 쓰는 식으로, 덧셈 기호를 뒤에 놓아봅시다.
이런 식으로 `1*(2+3)`을 다음과 같이 바꿀 수 있습니다.
...
즉 `1(23+)*`에서 괄호를 빼고 `123+*`라고 써도 의미가 전혀 애매해지지 않습니다.
...
<Alg>
  ...
  <ABlock>
    ...
    <ABlock>
      ...
      <AElif>토큰이 여는 괄호</AElif>
      <ABlock>
        $\textit{stack}$.push(토큰)
      </ABlock>

      <AElif>토큰이 닫는 괄호</AElif>
      <ABlock>
        <AUntil>스택 맨 위가 여는 괄호일 때</AUntil>
        <ABlock>
          $\textit{postfix}$.append(토큰)
        </ABlock>
      </ABlock>
