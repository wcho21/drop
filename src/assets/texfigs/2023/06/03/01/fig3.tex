...
\begin{document}
  \begin{tikzpicture}
    \begin{scope}
      ...
      \draw[Stlth] (0.5*\bw+0.5*\bbx,\bh+\bby+2mm) .. controls +(up:0.5*\arrh) and +(left:0.5*\arrw) .. ++(\arrw,\arrh);
    ...
    \begin{scope}[xshift=\bw+\bbx+\arrw+16mm,yshift=18mm]
      \node at (0,0) {A A B};
      \node at (0,-8mm) {\small (O)};

      \node at (20mm,0) {A B \textcolor{red}{B}};
      \node at (20mm,-8mm) {\small (X)};
    \end{scope}
  \end{tikzpicture}
\end{document}
