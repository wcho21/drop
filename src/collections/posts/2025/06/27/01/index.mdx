...
셸 스크립트<L>shell script</L>는 별도의 프로그램을 만들 필요 없이 기존에 있는 프로그램을 이용해 어떤 작업을 빠르게 만들 때 주로 사용되지만, 문법상 여러 특이한 점이 있어서 진입장벽이 다소 있기도 합니다.
그렇지만 셸 스크립트의 문법과 셸의 동작을 이해하고 나면 생각보다 어렵지 않게 사용할 수 있습니다.

여기서의 목표는, 셸 스크립트를 그때그때 필요한 부분만 복사해서 쓰는 대신에 제대로 익혀서 자유자재로 만드는 수준까지 나아가는 것입니다.
이를 위해 간단한 할 일 관리 프로그램을 직접 만들어보며, 각 단계마다 셸 스크립트의 문법을 하나씩 살펴볼 것입니다.
프로그램을 완성하고나면 변수 대입부터 조건문, 반복문까지의 기본적인 문법을 익히게 됩니다.
...
여기서는 셸을 어떻게 설치하는지는 다루지 않지만, 윈도우에서는 Cygwin 같은 환경을, 맥이나 리눅스에서는 기본적으로 설치된 Bash 셸을 그대로 이용할 수 있습니다.
...
앞으로 특별한 언급이 없는 한, 셸은 Bash 셸을 부르는 것으로 하겠습니다.
...
이제 보니 함수는 작은 프로그램처럼 보이기도 합니다.
함수 또한 `$1` 등으로 파라미터를 받고, `exit 1` 처럼 종료 코드를 리턴한다는 점을 주목해보세요.
...
이것은 다른 셸 스크립트에서 사용되기도 하므로 간략히 소개하고 넘어갑니다.
...
하지만 그 밖에도 셸에는 여러 기능이 있습니다.
셸 스크립트 작성에 필수적인 부분을 살펴보겠습니다.
